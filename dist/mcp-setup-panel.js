...
var McpSetupPanel = class McpSetupPanel {
	discovery;
	callbacks;
	options;
	done;
	...
	constructor(discovery, callbacks, options, tui, done) {
		...
		for (const entry of discovery.imports) this.selectedImports.add(entry.kind);
	...
	getActions() {
		...
		if (!this.discovery.sources.some((source) => source.id === "shared-project" && source.exists)) actions.push({
			id: "scaffold-project",
		...
		if (this.getDetectedPaths().length > 0) actions.push({
			id: "open-paths",
			label: "Open detected config paths",
			description: "Browse the actual config files that Pi discovered on this machine."
		});
		if (!this.discovery.repoPrompt.configured && this.discovery.repoPrompt.executablePath && this.discovery.repoPrompt.targetPath && this.discovery.repoPrompt.entry && this.discovery.repoPrompt.serverName) actions.push({
			id: "add-repoprompt",
			label: "Add RepoPrompt to shared MCP config",
			description: "Write a standard MCP entry for RepoPrompt to the recommended shared target, then reload MCP in-session."
		});
		actions.push({
			id: "close",
			label: "Close",
			description: "Exit the onboarding flow."
		});
		return actions;
	}
	getDetectedPaths() {
		const paths = [...this.discovery.sources.filter((source) => source.exists).map((source) => source.path), ...this.discovery.imports.map((entry) => entry.path)];
	...
	handleInput(data) {
		...
		if (this.keys.selectConfirm(data)) {
			...
			if (selected) this.runAction(selected.id);
		}
	}
	handleImportsInput(data) {
		...
		if (matchesKey(data, "space")) {
			...
			if (!current) return;
			if (this.selectedImports.has(current.kind)) this.selectedImports.delete(current.kind);
			else this.selectedImports.add(current.kind);
			this.tui.requestRender();
			return;
		}
		if (this.keys.selectConfirm(data)) this.applySelectedImports();
	}
	...
	async applySelectedImports() {
		...
		if (selected.length === 0) {
			this.notice = {
				text: "Select at least one compatibility import first.",
				tone: "warning"
			};
			this.tui.requestRender();
			return;
		...
		await this.runBusy(async () => {
			...
			this.notice = result.added.length > 0 ? {
				...
				text: `No changes needed in ${result.path}.`,
				tone: "muted"
			};
			this.screen = this.discovery.hasAnyConfig ? "setup" : "empty";
			this.actionCursor = 0;
	...
	render(width) {
		...
		if (this.notice) {
			...
			for (const line of wrapText(this.notice.text, innerW - 6)) lines.push(this.padLine(fg(tone, line), innerW));
			lines.push(this.padLine("", innerW));
		}
		lines.push(`├${border}┤`);
		if (this.screen === "imports") lines.push(...this.renderImports(innerW));
		else if (this.screen === "paths") lines.push(...this.renderPaths(innerW));
		else lines.push(...this.renderActions(innerW));
		lines.push(`└${border}┘`);
		return lines;
	}
	renderActions(innerW) {
		...
		for (let index = 0; index < actions.length; index++) {
			...
			const cursor = index === this.actionCursor ? fg(this.t.selected, "›") : " ";
		...
		for (const line of preview) lines.push(this.padLine(line, innerW));
		lines.push(this.padLine("", innerW));
	...
	renderImports(innerW) {
		...
		for (const line of this.formatWritePreview("Compatibility import write preview", preview)) lines.push(this.padLine(line, innerW));
		return lines;
	}
	renderPaths(innerW) {
		const lines = [];
	...
	discoverySummaryLine() {
		if (!this.discovery.hasAnyConfig) return fg(this.t.warning, this.options.onboardingState.setupCompleted ? "No MCP servers are active right now." : "No MCP config is active yet.");
		if (this.discovery.totalServerCount === 0 && (this.discovery.imports.length > 0 || !!this.discovery.repoPrompt.executablePath)) return fg(this.t.warning, "Pi found MCP-related setup options, but none are active in Pi yet.");
	...
	secondarySummaryLine() {
		if (!this.discovery.hasAnyConfig) return "Create a shared `.mcp.json`, adopt host imports, or quick-add RepoPrompt from this screen.";
		if (this.discovery.totalServerCount === 0 && this.discovery.imports.length > 0) return `Detected ${this.discovery.imports.length} compatibility import source${this.discovery.imports.length === 1 ? "" : "s"}. Adopt them into Pi or inspect the underlying files.`;
	...
	getActionPreview(action) {
		switch (action) {
			...
			case "show-precedence": return this.formatPreview([
				"Read order:",
			...
			case "open-paths": return this.formatPreview(this.getDetectedPaths().length > 0 ? ["Detected paths:", ...this.getDetectedPaths()] : ["No config paths were detected."]);
			case "add-repoprompt": {
				const repoPrompt = this.discovery.repoPrompt;
				const preview = this.callbacks.previewRepoPrompt();
				if (!preview) return this.formatPreview(["RepoPrompt is not available to add from this setup screen."]);
			...
			case "scaffold-project": return this.formatWritePreview("Starter project `.mcp.json` write preview", this.callbacks.previewStarterProject(), ["This writes a minimal `.mcp.json` in the current project using the shared MCP layout.", "It intentionally avoids adding a fake placeholder server that would fail on first reload."]);
			default: return this.formatPreview(["Close the setup flow."]);
		}
	}
	formatPreview(lines) {
		const preview = [];
		for (const line of lines) preview.push(...wrapText(line, 74));
		return preview;
	}
	formatWritePreview(title, preview, intro = []) {
		const lines = [];
		for (const line of intro) lines.push(...wrapText(line, 74));
		...
		for (const line of shown) lines.push(...wrapText(line, 74));
		if (diffLines.length > maxLines) lines.push(...wrapText(`… ${diffLines.length - maxLines} more diff line${diffLines.length - maxLines === 1 ? "" : "s"}`, 74));
		return lines;
	}
	padLine(text, innerW) {
		const inset = 2;
