...
describe("McpOAuthProvider", () => {
  ...
  describe("redirectUrl", () => {
    it("should return the correct redirect URL", () => {
      ...
      assert.strictEqual(provider.redirectUrl, "http://localhost:19876/callback");
    });
    ...
    it("should snapshot generated redirect URI at construction", () => {
      ...
      try {
        ...
        assert.deepStrictEqual(provider.clientMetadata.redirect_uris, [
          "http://localhost:41234/snapshot/callback",
        ]);
      } finally {
  ...
  describe("saveClientInformation", () => {
    it("should save client information", async () => {
      ...
      assert.deepStrictEqual(getAuthForUrl(serverName, serverUrl)?.clientInfo?.redirectUris, [
    ...
    it("should save the current redirect URL when registration omits redirect_uris", async () => {
      const provider = new McpOAuthProvider(
        "redirect-fallback",
        serverUrl,
        { redirectUri: "http://localhost:3118/custom" },
        {
          onRedirect: async () => {},
        },
      );
      ...
      assert.deepStrictEqual(
        getAuthForUrl("redirect-fallback", serverUrl)?.clientInfo?.redirectUris,
        ["http://localhost:3118/custom"],
      );
    });

    it("should return stored dynamic client info even when redirect URIs are stale", async () => {
      const provider = new McpOAuthProvider(
        "stale-redirect-client",
        serverUrl,
        { redirectUri: "http://localhost:3118/current" },
        {
          onRedirect: async () => {},
        },
      );
    ...
    it("should throw UnauthorizedError when no flow is in progress", async () => {
      ...
      await assert.rejects(
        ...
        (err: unknown) =>
          ...
          /Re-authentication required/.test((err as Error).message),
      );
    });
  ...
  describe("codeVerifier / saveCodeVerifier", () => {
    it("should save and retrieve code verifier", async () => {
      ...
      assert.strictEqual(
        getAuthForUrl("code-verifier-test", serverUrl)?.codeVerifier,
        "verifier-abc-123",
      );
    });
    ...
    it("should throw when no code verifier", async () => {
      ...
      await assert.rejects(async () => provider.codeVerifier(), /No code verifier saved/);
    });

    it("should ignore code verifiers saved for a different server URL", async () => {
      const provider = new McpOAuthProvider(
        "code-verifier-url-bound",
        serverUrl,
        {},
        {
          onRedirect: async () => {},
